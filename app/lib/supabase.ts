...
export const getMembers = async (teamIdentifier: string) => {
  const { data, error } = await supabase
    .from("Teams")
    .select("members")
    .eq("team_id", teamIdentifier)
...
export const getCorrectDB = async (teamIdentifier: string) => {
  const { data, error } = await supabase
    .from("Teams")
    .select("correct")
    .eq("team_id", teamIdentifier)
...
export const updateCorrectDB = async (teamIdentifier: string) => {
  const correct = await getCorrectDB(teamIdentifier);
...
export const updateCorrectList = async (
  ...
  const { error } = await supabase
    ...
    .eq("team_id", teamIdentifier); // insertではなくupdateを使用している点にも注意
  if (error) {
    throw new Error("Error inserting team info: " + error.message);
  }
};

export const getCorrectList = async (teamIdentifier: string) => {
  const { data, error } = await supabase
    .from("Teams")
    .select("correct_list")
    .eq("team_id", teamIdentifier)
